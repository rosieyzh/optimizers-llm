...
class Trainer:
    ...
    def load_trainer_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        for group in self.optim.param_groups:
            # Use name heuristics to detect norm layers/last layer.
            is_norm = self.cfg.optimizer.update_norm and ('norm' in group['param_names'][0] or 'ln_f' in group['param_names'][0])
            is_last = self.cfg.optimizer.update_last and 'ff_out_last' in group['param_names'][0]

            if (self.cfg.optimizer.name == OptimizerType.adafactorw_last or self.cfg.optimizer.name == OptimizerType.adalayerw_last or (self.cfg.optimizer.name == OptimizerType.adalayerw and self.cfg.optimizer.t_freeze and self.cfg.optimizer.lr_last)) and is_last:
                if self.cfg.optimizer.lr_last_multiplier is not None:
                    log.info("Using lr multiplier to set last layer group lr.")
                    lr_last = self.cfg.optimizer.learning_rate * self.cfg.optimizer.lr_last_multiplier
                else:
                    lr_last = self.cfg.optimizer.lr_last
                new_learning_rate = self.scheduler.get_lr(
                    lr_last, self.scheduler_current, self.scheduler_max
                )
                group["initial_lr"] = lr_last
                log.info(f"Resetting learning rate to {new_learning_rate} for last layer group.")
            elif (self.cfg.optimizer.name == OptimizerType.adafactorw_last or self.cfg.optimizer.name == OptimizerType.adalayerw_last or (self.cfg.optimizer.name == OptimizerType.adalayerw and self.cfg.optimizer.t_freeze and self.cfg.optimizer.lr_last)) and is_norm:
                if self.cfg.optimizer.lr_last_multiplier is not None:
                    log.info("Using lr multiplier to set last layer group lr.")
                    lr_last = self.cfg.optimizer.learning_rate * self.cfg.optimizer.lr_last_multiplier
                else:
                ...
                group["initial_lr"] = lr_last
                log.info(f"Resetting learning rate to {new_learning_rate} for norm layer group.")
            else:
                new_learning_rate = self.scheduler.get_lr(
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        for group in self.optim.param_groups:
            ...
            if (self.cfg.optimizer.name == OptimizerType.adafactorw_last or self.cfg.optimizer.name == OptimizerType.adalayerw_last or (self.cfg.optimizer.name == OptimizerType.adalayerw and self.cfg.optimizer.t_freeze and self.cfg.optimizer.lr_last)) and is_last:
                log.info(f"Getting lr for last layer using initial lr {group['initial_lr']}.")
    ...
    def should_eval_this_step(self) -> bool:
        ...
        if self.cfg.eval_interval is not None:
            return self.global_step % self.cfg.eval_interaval == 0
        else:
